		...
		try:
			...
			if self.passed and scoreUtils.isRankable(self.mods):
				...
				if b.rankedStatus == rankedStatuses.LOVED and glob.conf.extra["lets"]["submit"]["loved-dont-give-pp"]:
					personalBest = glob.db.fetch("SELECT id, score FROM scores_ap WHERE userid = %s AND beatmap_md5 = %s AND play_mode = %s AND completed = 3 LIMIT 1", [userID, self.fileMd5, self.gameMode])
				else:
